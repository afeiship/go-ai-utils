...
func main() {
	...
	if apiKey != "your-api-key" { // 只有在用户修改了API key时才执行
		clientWithKey := aiutils.NewClient(aiutils.NewClientOptions().WithAPIKey(apiKey))
		result, err = clientWithKey.Keywords(content)
		if err != nil {
	...
	// 方法6: 使用链式配置 + 完整选项
	fmt.Println("\n=== 链式配置Client ===")
	chainedClient := aiutils.NewClient(aiutils.NewClientOptions().
		WithModel("glm-4.5-air").
		WithMaxTokens(1500))

	result, err = chainedClient.Keywords(content, &aiutils.KeywordsOptions{
		Count:    5,
	...
	// 方法7: 自定义Client配置
	fmt.Println("\n=== 自定义Client配置 ===")
	customClient := aiutils.NewClient(aiutils.ClientOptions{
		APIKey:    "your-api-key", // 请替换为实际API Key
		BaseURL:   "https://api.anthropic.com",
		Model:     "glm-4.5-air",
		MaxTokens: 2000,
	})

	result, err = customClient.Keywords(content, &aiutils.KeywordsOptions{
		Count:    6,
		Language: aiutils.LanguageChinese,
	})
	if err != nil {
